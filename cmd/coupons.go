...
var couponsCreateCmd = &cobra.Command{
  ...
  mio coupons create --code SAVE20 --discount-type percentage --discount-value 20
  ...
  mio coupons create --code LAUNCH --discount-type percentage --discount-value 15 \
...
func init() {
	...
	couponsCreateCmd.Flags().String("discount-type", "", "Discount type: percentage or amount. Required.")
	...
	couponsCreateCmd.Flags().String("currency", "", "ISO 4217 currency code (required when discount-type=amount, e.g. USD).")
	couponsCreateCmd.Flags().Int("max-redemptions", 0, "Maximum number of times the coupon can be redeemed. Omit for unlimited.")
	...
	couponsUpdateCmd.Flags().Int("max-redemptions", 0, "Maximum number of redemptions. Set to 0 to clear the limit.")
	couponsUpdateCmd.Flags().Bool("first-time-only", false, "Restrict to first-time buyers only.")
	couponsUpdateCmd.Flags().String("expires-at", "", "Expiry timestamp in RFC 3339 format, or empty to clear.")
