...
- **Auth resolution (first wins):** `--api-key` flag → `MIO_API_KEY` env → key stored by `mio login`. No key ⇒ exit `3`.
  On macOS (and Linux without a desktop Secret Service/KWallet) the stored key is an encrypted FILE under the config dir (`$XDG_CONFIG_HOME/mio/keyring/api-key`, else `~/.config/mio/keyring/api-key`; the macOS release binary never uses the Keychain), so a shell with a different `XDG_CONFIG_HOME` or `HOME` sees no key. Windows uses the Credential Manager. The exit-3 error names the store it read, and `mio whoami` reports it as `key_source`. Export it once instead of reading the store on every call: `export MIO_API_KEY="$(mio auth token)"` (stored key only, to stdout; exit `3` with empty stdout when none is stored).
