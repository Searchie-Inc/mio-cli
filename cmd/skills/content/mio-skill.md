...
- **Auth resolution (first wins):** `--api-key` flag → `MIO_API_KEY` env → key stored by `mio login`. No key ⇒ exit `3`.
  With the release binaries, on macOS (and on Linux without a desktop Secret Service/KWallet) the stored key is an encrypted FILE under the config dir (`$XDG_CONFIG_HOME/mio/keyring/api-key`, else `~/.config/mio/keyring/api-key`; the macOS release binary never uses the Keychain), so a shell with a different `XDG_CONFIG_HOME` or `HOME` sees no key. A macOS build from source with cgo on (the `go install` default when a C toolchain such as Xcode's is present) uses the Keychain instead; Windows uses the Credential Manager. The exit-3 error names the store it read, and `mio whoami` reports the one in use as `key_source`: check it rather than assume. Export it once instead of reading the store on every call: `MIO_API_KEY="$(mio auth token)"; export MIO_API_KEY` (two statements: `export MIO_API_KEY="$(…)"` returns 0, so `set -e` never sees the exit; stored key only, to stdout; exit `3` with empty stdout when none is stored).
- **Locked out (lost password, no browser)?** `mio auth forgot-password --email <addr>`, then `MIO_PASSWORD='<new>' mio auth reset-password --token '<the emailed link, quoted, or the token after its #>'`, then `mio login`. Both send no key (they work from the locked-out session); the new password is never on argv; a 410 means the token expired (one hour) or was used — run `forgot-password` again (MIO-4286; release after `v0.24.0`, needs mio-backend MIO-3571 on the API).
...
- **A `--hub` run fills gaps; it does not overwrite (MIO-4166).**
  ...
  and a bucket the template lacks, like `mobile`, is never dropped), and so are its policy
  text, its policy gate and its onboarding configs. Flags on the command itself
  (`--primary-color`, `--logo-url`, `--registration-enabled`, …) still win over the hub.
...
- **Legal policies come with their enforcement switch.**
  ...
  one per policy.
  **A `--hub` run never resets a policy (MIO-2818):** it writes a template's policy text
