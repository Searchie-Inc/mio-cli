...
- **Capturing a STRING id? Add `-o plain` (MIO-2792).** `--jq` renders through the JSON formatter, so a string result comes back **JSON-quoted** and `$(…)` captures the quotes: `HUB_ID=$(mio hubs list --jq '.[0].id')` yields `"019f…"`, which then 404s when you pass it to the next command. `-o plain` prints the bare scalar:
  ...
  A `--jq` that yields several scalars prints **one bare value per line** under `-o plain`, whatever the count, and nothing when it yields none (objects stay `key=value` blocks): `mio products list -o plain --jq '.[].id' | while read -r id; do …; done` (MIO-4174).
- **A list returns one page (usually 20 rows).** When the API reports more, a one-line `note:` on **stderr** names the flag and cursor for the next page: `--after <cursor>` (`--page-after` on `segments search`; `--limit` on `media search`, which returns no cursor). stdout stays the bare array. Before concluding a record doesn't exist, check stderr for that note (MIO-4174).
