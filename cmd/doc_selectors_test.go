...
//     — must yield the attribute it names in at least one output mode of the
//     REAL renderer (internal/output.Render over a flattened and a raw
//     resource and collection). The oracle is the renderer, not a rule about
//     spellings: `.attributes.x` fails because Render never emits a top-level
//     `attributes`, and would pass the day it did.
...
// single resource is {"data": …}; a list adds links and meta).
func selectorOutputModes(t *testing.T, path string, attrs map[string]any, want any, raw bool) []string {
	t.Helper()
	...
	modes := []mode{
		{"default output, one resource", &res, false},
		{"default output, a list", &col, false},
	}
	if raw {
		modes = append(modes, mode{"--raw, one resource", &res, true}, mode{"--raw, a list", &col, true})
	}
...
// when an output mode the text names answers it; otherwise it says why sel is
// broken.
func attributesSelectorProblem(t *testing.T, sel docexamples.Selector) string {
	t.Helper()
	...
	if len(selectorOutputModes(t, sel.Path, attrs, want, raw)) > 0 {
		return ""
	}
	...
	return fmt.Sprintf("selector %s yields nothing in %s — the default output flattens a resource's attributes "+
		"to the top level, so select .%s (capture a string with -o plain), or say --raw and use .data.attributes.%s",
		sel.Path, modes, leaf, leaf)
}

// TestDocSelectors_AttributesPathsResolveInSomeOutputMode is the MIO-3413
// class guard: no text the CLI prints or ships may name a path through
// `attributes` that the renderer answers with nothing.
func TestDocSelectors_AttributesPathsResolveInSomeOutputMode(t *testing.T) {
	...
	for _, src := range sources {
		for _, sel := range docexamples.Selectors(src.file, src.firstLine, src.text) {
			...
			probed++
			if p := attributesSelectorProblem(t, sel); p != "" {
...
func TestDocSelectors_ProbeTellsBrokenFromWorking(t *testing.T) {
	...
	broken := []string{
		"the .attributes.contact_id field from 'mio contacts'",              // MIO-3413 as shipped
		"mio contacts retrieve <id> -o plain --jq '.attributes.contact_id'", // its capture
		"mio contacts retrieve <id> --raw --jq '.attributes.contact_id'",    // --raw needs .data
		"mio contacts list --jq '.[].attributes.contact_id'",
		"filter client-side on .attributes.status",
		`.attributes["contact_id"]`,
		"the raw path .data.attributes.contact_id", // the right path, but --raw is never named
	}
	for _, line := range broken {
		if p := attributesSelectorProblem(t, one(line)); p == "" {
			t.Errorf("%q: the probe accepted a path that yields nothing where the text says to use it", line)
		}
	}
	working := []string{
		".data.attributes.contact_id under --raw",
		"mio contacts list --raw --jq '.data[0].attributes.contact_id'",
		"mio contacts list --raw --jq '.data[].attributes.contact_id'",
		"mio hubs retrieve --raw --jq .data.attributes.settings.registration.enabled",
		"mio contacts retrieve <id> --raw --jq '.[].attributes.contact_id'", // .[] over {"data": …}
	}
	for _, line := range working {
		if p := attributesSelectorProblem(t, one(line)); p != "" {
			t.Errorf("%q: the probe rejected a path the renderer answers: %s", line, p)
		}
	}
}
