...
| `content reconcile` | Materialise content items for a hub's media playlists (one container per playlist, one lesson per item) — a file that lives only in a playlist has NO content item, so progress/completion tracking, "My List", comments and single-file page bindings are all missing for it. Additive and safe to re-run. With no `--playlist-id` it reconciles the playlists the hub was SCAFFOLDED with, from its `HubTemplateApplication` provenance — a hand-built hub has no such row, so a bare run is REJECTED with `422 no_playlist_provenance` (exit 2) rather than doing nothing quietly; it never falls back to "every playlist on the hub". Pass `--playlist-id` explicitly there. Created lessons land unpublished unless the file AND playlist are each already published |
