...
| `auth` | `token` — print the key `mio login` stored, and nothing else, to stdout: `export MIO_API_KEY="$(mio auth token)"`. Reads only the store (not `--api-key`/`MIO_API_KEY`), no request; exit 3 with empty stdout when none is stored (MIO-2995; release after `v0.22.0`) |
