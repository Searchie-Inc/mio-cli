...
- **`products prices` takes the product id as a positional argument**, not `--product`:
  ...
  `--amount`, `--currency` and `--type` (`one_time`|`recurring`) are all required; `--interval` **and** `--interval-count` are required when `--type=recurring`. The optional label flag is `--name` (there is no `--nickname`), alongside `--description`, `--is-active` and `--trial-period-days` (free-trial days, ≥0, 0 = no trial; checkout applies it to a `recurring` price only — the API also stores it on a `one_time` price, where it does nothing). Amount, currency, type, interval, interval count and trial length are **fixed after create** (`prices update` takes only `--name`/`--description`/`--is-active`): to change one, create a new price and deactivate the old one (MIO-4159).
