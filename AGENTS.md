...
Under `-o plain`, a `--jq` result that is a stream or an array of scalars (strings, numbers, booleans; a null prints an empty line) prints **one bare value per line, whatever the count**, and a filter that yields nothing prints nothing. A string is printed verbatim, so one that itself holds a newline (policy `content`, say) spans several lines and shifts the values after it: use `-o json` for those, where each is one JSON-escaped string. Objects, and arrays holding objects or arrays, still print as `key=value` blocks separated by a blank line.

**A list command returns one page** — the API's default size, usually 20 rows. When the API reports more, a one-line `note:` on **stderr** names the flag and cursor for the next page: `--after <cursor>` (`--page-after` on `segments search`; `--limit` on `media search`, which is top-N and returns no cursor). `checkout orders|subscriptions|payments|webhooks list` report nothing either way, so there the note fires on a **full** page (20 rows, or `--limit`) and says only that more may exist; an empty next page means there were none. At the API's page-size cap (`--limit 100` on most lists; 200 on `segments members` and on `segments search --page-size`) the note stops suggesting a larger page. A cursor a shell would split (discussions hand out `<timestamp>|<id>`) is single-quoted, so the suggestion pastes as-is. stdout is unchanged: still the bare array, so `.[0].id` and `.[].id` keep working. `--raw` prints no note: it prints the API's own envelope, and lists signal more rows three ways — `has_more` in `meta` or `meta.page`; only a `links.next` on the media lists (`media files|attachments|playlists|playlists items|hub-media|hub-playlists list`, which send no `meta`; files and attachments send one on every full page, so there the note says only that a next-page link came, and that page can be empty); or nothing at all (the four checkout lists above). The cursor is in `next_cursor` or `links.next` where the API sends one; a list that has no place for one (`achievements offerings list`, the four checkout lists) takes its last row's id as `--after`, which is what the note offers.

> **Version gate (MIO-4174).** Both land in the release AFTER `v0.23.0`. On `v0.23.0` and earlier, a `-o plain --jq` stream of two or more values prints `value=X` records separated by blank lines (one value prints bare, zero prints an empty line), and nothing tells you a list has more rows: read `meta` and `links` under `--raw` (the media lists signal only with `links.next`; the four checkout lists signal nothing, so page them until a page comes back short).
...
| `auth` | `token` — print the key `mio login` stored, and nothing else, to stdout: `MIO_API_KEY="$(mio auth token)"; export MIO_API_KEY` (two statements, so `set -e` sees the exit 3). Reads only the store (not `--api-key`/`MIO_API_KEY`), no request; exit 3 with empty stdout when none is stored (MIO-2995; release after `v0.22.0`). **`forgot-password` / `reset-password` (MIO-4286)** — recover a lost PLATFORM-user password (the account `mio login` signs in) from the CLI alone, with no browser. Both are UNAUTHENTICATED and send NO key, even one stored or exported, so they work from the very session whose key is stale or revoked. `forgot-password --email <addr>` POSTs `/api/v1/auth/forgot` and prints the neutral "if an account exists for `<addr>`, a reset email is on its way" on exit 0 whatever the address (the API never reveals whether an account exists, and neither does the CLI); off a TTY it needs `--email` (exit 2, no request); 429 is exit 6 (5 an hour per IP); a 500 here is the deployment having no reset landing page configured, not a transient failure (hinted). `reset-password --token <token\|link>` POSTs `/api/v1/auth/reset` with the token and the new password: `--token` takes the bare token or the WHOLE emailed link, quoted (it contains `#`), and sends the part after `#` (the URL fragment, where the backend puts it); a link with nothing after `#` exits 2 BEFORE any request (a mail client stripped the fragment). The new password is NEVER on argv (no `--password` flag): prompted twice on a TTY, else read from `MIO_PASSWORD` (the variable `mio login` reads too), else exit 2 with no request. 204 → exit 0, "Password reset … Next: mio login": the reset mints and stores NOTHING and signs the account out of every session (an API key already stored keeps working). 410 → exit 1 with the hint to run `forgot-password` again; the token expired (one hour), was used, was expired by another link being redeemed (asking again does not cancel an earlier link; redeeming one expires the rest), or is unknown, and the API says which to nobody, so branch on `errors[0].code` = `password_reset_expired`. 422 (password under 8 characters) → exit 2. 404/405 on either command → the API has no password-reset route yet (mio-backend MIO-3571; hinted). **Version gate:** both land in the release AFTER `v0.24.0` (`unknown command` before) and need mio-backend MIO-3571 on the API |
...
| `skills` | `install` `print` — `install` writes the embedded agent skill to ONE scope: `--user` (default; `~/.claude/skills/mio/SKILL.md`, or `$CODEX_HOME/skills/mio/SKILL.md` with `--target codex`) or `--project` (`./.claude/skills/mio/SKILL.md`, `./.codex/skills/mio/SKILL.md`). `mio update` refreshes an unmodified copy in the user scope and in the project scope of the CURRENT directory only, so a project copy is refreshed only when `mio update` runs from that project; a hand-edited copy is never overwritten, and stderr names it with the exact `mio skills install [--project] --force --target <t>` command. `install` warns on stderr when the other scope holds an older managed copy. **Version gate (MIO-4178):** both land in the release after `v0.23.0`, and the refresh runs in the binary DOING the update, so project copies are covered only when updating FROM that release or later — the update onto it still leaves them alone; refresh each once with `mio skills install --project --target <t>` from its directory |
| `config` | `set` `get` `list` |
| `api-keys` | `create` `list` `retrieve` `delete` |
| `teams` | `create` `list` `retrieve` `update` `delete` `switch` — **`create` needs `--name` AND `--slug`** (the only two fields the API takes; a taken slug is 409, exit 2) **and a USER access token**: the API answers an API key, including the stored one, with 403 (exit 3), so pass the `access_token` from `POST /api/v1/auth/login` via `--api-key`. `delete` and `switch` refuse an API key the same way (403, exit 3), even one bound to the target team. `update` renames only (`--name`); a slug is fixed at creation. **Version gate (MIO-3830):** `--slug` lands in the release AFTER `v0.23.0`; on `v0.23.0` and earlier `teams create` sends no `slug` (and `subdomain` when `--subdomain` is passed), so it always answers 422. `--subdomain` survives as a hidden, deprecated alias of `--slug` |
...
| `hubs policies` | `get` `update` `gate` — `update` writes a policy document, `gate` flips the hub-level enforcement switch (`settings.policies.enabled`). They are SEPARATE writes: content without the gate is a hub whose ToS exists but is never presented (MIO-2020, MIO-2567). **`get` (MIO-2815)** is the team-owner ADMIN read — the same path as the `update` PATCH, so the method is the only discriminator — reporting both documents and the gate **as stored**. It is not interchangeable with the member-portal read, which serves defaults and forces `enabled=true`. Returns a LIST of `{policy_type, content, version, enabled, require_acceptance}`. Always returns exactly TWO items (tos, privacy_policy) — an unconfigured document is served with rendered default text, so there is NO empty case. `enabled` is the ONE hub-level gate repeated per item. **`version` is NOT a custom-vs-default discriminator**: the backend assigns a version ONLY for tos AND ONLY when require_acceptance is set, and projects an absent one as "default-v1" — so a hand-written privacy policy reads "default-v1". `content` always has a value (yours, else the rendered default) and `require_acceptance` is tri-state (null = unknown, not false). READ THE CONTENT and look at it — that is the only reliable check this read offers (`hubs scaffold --hub` does not depend on it: it reads the RAW `settings.policies` off the hub GET, where an unconfigured document has no content at all, MIO-2818). Capture the gate with `-o plain --jq '.[0].enabled'`; a bare `--jq .enabled` EXITS 1 ("expected an object but got: array"), it does not yield null. A `hubs scaffold --hub --reapply-template` run (and any `--hub` run on `v0.23.0` or earlier) sends `content:null` for any policy its template declares no text for and reverts that document. Whether that RE-PROMPTS members is decided by THAT write, not by how the tos was last saved: the backend moves the tos version only when the write itself carries `require_acceptance: true` (the scaffold sends that only when the template's tos declares `required: true` or `require_acceptance: true` — the shipped `community` one does) AND the effective version changes. A reset targets `default-v1`, so it re-prompts exactly when this read reports a tos `version` OTHER than `default-v1` — a `v_…` token, set by a custom-text save WITH `--require-acceptance` and KEPT by any later save or reset without the flag. So `version` is the re-prompt signal even though it is not the custom-vs-default one. At `default-v1`, or for a template tos that does not declare acceptance `true`, the text is replaced with no re-prompt for members who had accepted and no version change to notice afterwards (a member who NEVER accepted is prompted whenever the gate is on, whatever the version) |
| `hubs branding` | `attach` — attach an uploaded media file as a managed branding asset (`--file-id`, `--role logo|favicon|social_image|auth_logo`, optional `--position`; hub id optional positional). Resolves the file's `media_id`, preflights publish eligibility CLIENT-SIDE mirroring the backend predicate (READY + raster image; SVG rejected — the backend would accept the attach yet never publish it, a silent no-op), POSTs `hub-branding-attachments`, then re-reads the hub and surfaces the overlaid `branding.<role>_url` as a derived `resolved_public_url` attribute (stderr warning when unresolved). Replace-on-attach per (hub, role) |
| `hubs scaffold` | Build a full-experience hub in one idempotent command from a hub template in the target backend's LIVE catalog (`--template <id>` — the CLI embeds none; `--catalog <file>` is the digest-verified escape hatch, fail-closed on mismatch, and there is no `--offline`): branding/favicon, menu, registration, discussion spaces, onboarding schema, policies, playlists, pages, and — when the template declares a `welcomePost` — a welcome discussion (MIO-2558; no shipped catalog declares one yet, so that terminal step is a plan-visible no-op today, and it is idempotent by post title — compared and posted STRIPPED, because the API stores `title.strip()` — so a resume never creates a second one; the json result reports `welcome_post_status` as `created`/`adopted`/`adopted_deleted`. **`adopted` does not guarantee the post is visible**: the admin list does not serialize `is_removed`, so a moderation-REMOVED match is indistinguishable from a live one and reports as plain `adopted`. Only `adopted_deleted` (soft-deleted) is detectable. If it matters, read the post back with `mio community discussions retrieve`). **SERVER-SIDE HUB OP (MIO-2976).** In CREATE mode the run first PROBES `POST /api/teams/{team_id}/hubs/from-template` (type `hub_scaffolds`, MIO-2926) — the probe IS the real POST, never a capability check. When it answers, the backend builds the WHOLE hub in one transaction and none of the ten steps below run. **The op's flag (`HUB_SCAFFOLD_ENABLED`) defaults off and is set per deployment, so whether a run takes the op depends on the backend** — one with it off answers the dormant signal below and the run falls back, noted on stderr. Fallback happens on the dormant/absent signal ONLY: a bare `405` with `Allow: GET`, which is byte-identical to a backend without the route. Note this op ALSO answers `404 template_not_found`, and 404 and 405 both derive exit 4 — so the fallback keys on a dedicated `client.ErrHubOpAbsent` sentinel, never on the exit code; every other status surfaces, because a client-side apply against a backend that HAS the op just smears partial state. ALWAYS client-side: `--hub` (the op is create-only), `--dry-run` (the op has no plan mode — probing it would create the hub), an invocation missing `--name` or `--slug` (the op requires both non-empty and will not mint a slug), any branding override (`--branding-json` or a palette flag) and `--catalog` — the op's `overrides{}` carries only `logo_url`/`favicon_url`/`registration_enabled`/`publish`, so taking it with an inexpressible flag would silently drop what was asked for. An empty (or whitespace-only) value for ANY branding key ending `_url` — however spelled: `--logo-url`, `--favicon-url`, `--social-image-url`, or `--branding-json '{"logo_url":""}'` — is a USAGE ERROR before any request (exit 2): the API rejects an empty branding `*_url` on hub create AND hub update (`validate_branding` → `_validate_absolute_https_url`, which rejects the empty string, wired to both write schemas), so NEITHER path can honour it — routing it client-side would create the hub and then 422 at the blobs PATCH, leaving a half-built hub with no rollback. To clear a branding key, scaffold without the flag and then `mio hubs update <hub_id> --unset branding.logo_url`. Every FLAG-forced skip is noted on stderr; `--dry-run` is not, since it is structural and the plan renderer emits no apply-time notes. **Template vocabulary no longer forces the client path (MIO-4167).** MIO-3065 kept a template declaring `spaces[].icon`, `playlists[].documents` or a page node binding a playlist `dataSource` by `key` off the op, which modelled none of them then. The op has applied all three since mio-backend #680 (MIO-3073, 2026-08-11) — the icon, each document as a synthetic file published to the hub and attached in order, and the binding filled from the ids its own playlists step created — so on a backend with the op enabled such a template, `starter` included, now TAKES the op. **Version gate:** on `v0.23.0` and earlier the CLI still skips the op for that vocabulary, announcing `(mio-backend parity: MIO-3073)`. The PAGES-only op (probed on the client path) still skips a page that binds a playlist by key, announced: it fills a binding only by matching the playlist's TITLE on the hub and writes a blank band marked applied when the title is ambiguous, while the client-side step holds the exact ids. IDEMPOTENCY: the `Idempotency-Key` is derived deterministically from team+template+name+slug, so re-running the same command converges on the stored application instead of creating a second hub (MIO-2565 on this path). The backend's fingerprint additionally covers the catalog digest and the overrides, so the same name+slug after a catalog pin move — or with different override flags — is `409 idempotency_fingerprint_mismatch`, and the CLI's error text carries that token in square brackets so an agent can branch on it (after `v0.23.0`, branch on `errors[0].code` instead: it carries the token in every mode, while `--raw` puts the API's own `detail` on stderr, without the bracketed token — MIO-3912): nothing was applied, and the CLI says so and names the way out (different identity, or `--catalog` to force the client path). RESULT PARITY: `-o json` reports the same keys on both paths. The op returns ids as `created_resource_ids` (per-KIND ordered lists, no slug) plus an ordered `summary[]` of `{resource, action, reason?}` rows whose `resource` IS slug-scoped (`space:<slug>`, `page:<slug>`, `playlist:<key>`, `onboarding:<slug>`, plus `hub`/`welcome_post`; a template with playlists also yields `document:<key>:<index>` rows, one per synthetic document file, whose ids land in `created_resource_ids.files` and are not part of the CLI result, and `content_node:<legacy_hash>` rows for the content items the op materialises); the two are paired positionally, which is sound because the backend appends to both in one statement — but the pairing is CHECKED, and any kind whose created-row count disagrees with its id count is dropped to `null` rather than risking a wrong id. Skipped rows become stderr notes; a `replayed:true` response carries the summary but NO ids and is disclosed as a replay; and the finished hub is read back with a GET so the reported `hub_slug`/`hub_path`/`published` are OBSERVED (the backend mints a unique slug when the requested one is taken), degrading to the requested values with a note if that read fails. The whole plan is validated before any write (`--name` ≤255 code points; `{{hub_name}}`/`{{hub_slug}}` interpolation — unknown tokens rejected, capped post-substitution). Pages: probes the backend `scaffold-from-template` op, falling back client-side on 404/405 (op absent or path shadowed on older backends: create with a `meta.template_provenance` marker → tree set → publish → mark applied), interpolating with the hub's actual title/slug. Re-runs with `--hub <id>` resume by FILLING GAPS (MIO-4166): a branding/settings key the hub already has keeps its value (a null counts as missing), a navigation bucket it has is kept whole (only an absent or empty bucket is filled, and a bucket the template lacks, e.g. `mobile`, is never dropped), and so are its policy text, its policy gate and its onboarding hub-configs; flags on the invocation (palette flags, `--branding-json`, `--logo-url`, `--favicon-url`, `--registration-enabled`) still win over the hub, so the printed `Resume with:` command keeps working; every kept value is named on stderr and in the `--dry-run` plan, and no hub PATCH is sent when nothing is missing (the template's `settings.policies` is never part of the fill: the hub PATCH drops it, and the gate is the policies step's). Filling a bucket sends the hub's own buckets back as stored, NOT re-checked against the CLI's hub-scoped href rule — but the API refuses that write while one of them links a page that is not one of the hub's active pages (a deleted one, typically); the run then exits 2 and names `hubs navigation remove`. `--reapply-template` restores the template-wins apply for the blobs, the policies and the onboarding hub-configs (the template's branding/settings keys win and keys it does not set survive, navigation replaced whole, every template policy written; the gate stays ENABLE-ONLY — turned on when the template declares it, never off) — NOT for pages, spaces or playlists: it is DESTRUCTIVE — exit 5 off a TTY without `--yes`, before any request (`--dry-run` needs no confirmation) — and a usage error (exit 2) without `--hub`. **Version gate: on `v0.23.0` and earlier every `--hub` run overwrites the palette, menu, settings and policy text.** An edited or foreign page at a template slug (or any pre-existing homepage) exits 2 — never overwritten — and on `--hub` that check runs BEFORE any write, on EVERY `--hub` run (`--reapply-template` and `--dry-run` included): no flag overwrites a page, so resolving the conflict is the operator's job. **Playlists (MIO-3065).** Each is created SCOPED to the hub (`hub_id`) — without that its hub detail page 404s for every viewer whatever the publication row says — at the template's own `visibility` (validated against the enum the CREATE takes, `public|unlisted|private`; the hub-media enum's `members` is NOT a legal value here and used to pass preflight and 422 at apply). Its per-hub publication row is `visibility: public`, a ratified hardcode with no manifest key behind it: members-only day one is not template-expressible today. A `playlists[].documents[]` entry becomes a synthetic READY document file (`files/synthetic`, MIO-2285) at the playlist's own visibility, gets its OWN per-hub publication row — without which it is attached but discloses zero item cards to an anonymous viewer — and is attached as an item; items land `file_ids` first, then documents. Files named in `file_ids[]` are attached but NOT published: publishing media the author already owns is not the scaffold's call. **Playlist dataSource FILL CONTRACT (MIO-3065).** A page node shipping `dataSource: {"type":"playlist","id":"","key":"<playlists[].key>"}` has that `key` resolved against the playlists this run created and the id written into `id` before the tree PUT — the renderer filters on a truthy `ds.id`, and the backend compiler treats a present-but-empty id as `("container","")`, so an unfilled node renders an empty band. A key naming no playlist of the same template is an exit-2 PREFLIGHT failure, before the hub exists. A key that survives to apply time unfillable — a resume whose playlists step skipped (the create-only gate) and whose id could not be recovered — fails the WHOLE pages step with exit 2 BEFORE any page is written: writing it would publish a blank band and then mark the page `applied`, after which §5.1 reads it as converged and no re-run repairs it. The ordinary resume does not hit that: on the skip, the step recovers the ids the plan needs from the hub's existing playlists, joined by TITLE and only where that title is unambiguous across the whole template AND on the hub (`key` is manifest vocabulary and is never stored server-side, so a wrong id would be acted on where a missing one is merely reported). **Content items (MIO-4167).** Between playlists and pages — where the op runs its own content step — the client-side pipeline POSTs `…/hubs/{hub}/content/reconcile` with EXPLICIT `playlist_ids`: the ids this run created, or recovered on a resume, in template order (never a bodyless POST — a client-side scaffold records no `HubTemplateApplication` provenance — and never a title guess). A playlist it holds no id for is NAMED on stderr with the `mio content reconcile` command that fixes it, and so is a recovered one that is not scoped to the hub (a team-library playlist published into it — reconcile rejects the whole request for one with `422 playlist_not_in_hub`; the page binding still uses it); a backend without the route (405/404) is a stderr note, not a failure; any other failure stops the run and names that command with every id. The json result reports the outcome as `content_nodes`: one `{legacy_hash, outcome, node_id}` per content item on both paths (`outcome` is the backend's vocabulary — `created`/`adopted`/`adopted_not_member_visible`/`skipped_*`; `node_id` null when unknown), `[]` when the template has no playlists, null when the run did not reconcile. **Version gate:** the step and the `content_nodes` key land in the release AFTER `v0.23.0`; on `v0.23.0` and earlier a client-side scaffold creates no content items and the key is absent. **Policies (MIO-2567):** the step writes each policy document AND then flips the hub-level enforcement gate (`PATCH …/policies/gate`, `settings.policies.enabled`) when the template declares `enabled: true`. Enforcement is ONE flag per hub, not one per policy, so the template's per-policy `enabled` values are collapsed onto it — they must agree, and a `true` alongside a `false` is rejected in the WRITE-FREE PREFLIGHT (exit 2 naming both keys, before the hub is created). The write is ENABLE-ONLY, matching the ratified applier contract: a template that declares none, or that resolves to `false`, writes no gate and leaves the hub's setting as it is (narrated on stderr) — disabling is `mio hubs policies gate <hub_id> --enabled=false`. **POLICIES ON `--hub` (MIO-2818).** A `--hub` run never sends `content:null`: it writes template text only to a policy whose STORED content is null or absent — the backend's own serve-the-default predicate, read raw off the hub GET — and keeps any text the hub has; it also keeps a gate the hub has already set (a JSON boolean, on or off) and fills only an unset one. A template-wins apply (create, or `--reapply-template`) always sends `content`, so a template that omits it (the shipped `community` one does) RESETS that policy to the backend default. For a tos whose template declares `required: true` or `require_acceptance: true` (the `community` one does) that reset also moves the version to `default-v1`, which RE-PROMPTS every member who had accepted — but ONLY when the hub's tos carried another version (`mio hubs policies get` shows a `v_…` token: set by a custom-text save WITH `--require-acceptance`, kept by later saves or resets without it); at `default-v1` nothing moves and nobody who had accepted is asked again (a gate this run turns ON still prompts every member who NEVER accepted — that is the gate, not the version). Template tos TEXT that differs from the stored text gets a fresh version and re-prompts too; a template tos that does not declare acceptance `true` never moves the version. If you hand-edited a hub's ToS, re-apply it after a `--reapply-template` run (or any `--hub` run on `v0.23.0` or earlier) (`mio hubs policies update <hub_id> --policy-type tos --content @tos.md --require-acceptance`), or read the stored state first with `GET /api/v1/teams/{team_id}/hubs/{hub_id}/policies` (the admin policies READ, MIO-2394 — same owner credentials as `policies gate`; read it with `mio hubs policies get <hub_id>`, MIO-2815). **Version gate: the gate write and the `policy_gate` result key are NOT in `v0.13.0` or earlier** — on those binaries a scaffolded hub's ToS is written but never enforced (a fresh member reads `tos_acceptance_required:false` and `POST …/tos/accept` answers the enumeration-safe 404); repair with `mio hubs policies gate <hub_id> --enabled`. `--dry-run` previews the plan (including the palette it would apply, and the gate line); `--publish` (default off) goes live; `--name`/`--slug` create a new hub; `--favicon-url`/`--logo-url`/`--registration-enabled` override the template. **Version gate: `--primary-color` and friends, and the machine-readable `--output json` result, are NOT in `v0.12.1`** — on that binary they exit 2 with `unknown flag`. Check `mio version`. **Branding (MIO-2604):** `--primary-color`/`--secondary-color`/`--text-color`/`--background-color`/`--header-color`/`--header-accent`/`--social-image-url` (plus `--branding-json` for a whole object) all MERGE over the template's `branding` block, so a key you don't name keeps the template's value; precedence is template → `--branding-json` → scalar flags. `--primary-color` also fills `header_color` unless YOU gave a header color (`--header-color`, or a `header_color` key in `--branding-json`) — the template's own value does not suppress it. Values are passed through unvalidated (branding is opaque JSONB server-side); the KEYS are strict-checked pre-auth against the MIO-2515 allowlist, so a typo exits 2 with no request. Honors `--output` like every other command: json (the default off a TTY) returns `{hub_id, hub_slug, hub_name, hub_path, published, template_id, catalog_revision, branding_overrides, homepage_page_id, welcome_post_id, welcome_post_status, pages[], spaces[], onboarding_attributes[], playlists[], policies[], policy_gate, content_nodes}` — `branding_overrides` is the resolved override layer, cascade included (`{}` when none), and `policy_gate` is the enforcement gate this run WROTE — `true`, or `null` when it wrote none (no declaration, or a resolved `false`, both of which leave the hub's setting standing; the write is enable-only; on `--hub`, also a gate the hub had already set). `policies[]` says the document exists, `policy_gate` says whether this run turned enforcement on — so `HUB_ID=$(mio hubs scaffold … -o plain --jq .hub_id)` works (`-o plain`: `--jq` alone JSON-quotes a string, MIO-2792); all progress narration goes to stderr, and a step that fails after the hub exists names the created hub id in the error (nothing is rolled back). |
...
| `content` | `create` `list` `retrieve` `children` `update` `delete` `restore` `reorder` `reconcile` — media binding on create/update takes EITHER `--file-id` (the id from `media files upload`/`list`; its media_id is resolved for you — **prefer this**) OR `--media-id` (the Media PK; the backend does NOT validate it, so a file id passed here stores verbatim and yields a lesson pointing at nothing). Mutually exclusive, and an EMPTY value for either is a usage error rather than an unlink — `--media-id "$VAR"` with `$VAR` unset would otherwise destroy a working link and exit 0. Unlink deliberately with `--unset-media` (a boolean no variable can expand into); it is **destructive** — it prompts, so a non-interactive shell needs `--yes` or it exits 5 having sent nothing |
| `content reconcile` | Materialise content items for a hub's media playlists (one container per playlist, one lesson per item) — a file that lives only in a playlist has NO content item, so progress/completion tracking, "My List", comments and single-file page bindings are all missing for it. Additive and safe to re-run. With no `--playlist-id` it reconciles the playlists in the hub's `HubTemplateApplication` provenance, which ONLY the server-side whole-hub op records (`hubs scaffold` in create mode, when the backend op built the hub) — a hand-built hub has no such row, and neither does any CLIENT-SIDE scaffold (`--hub` resume, a palette/`--branding-json` flag, `--catalog`, a missing `--name`/`--slug`, a backend with the op off), so a bare run there is REJECTED with `422 no_playlist_provenance` (exit 2) rather than doing nothing quietly; it never falls back to "every playlist on the hub". Pass `--playlist-id` explicitly there (`hubs scaffold -o json` reports them as `.playlists[].playlist_id`; later, `media playlists list --limit 100 -o json --jq '.[] | select(.hub_id == "<hub>") | {id, title}'` lists the ones scoped to the hub — not `media hub-playlists list`, whose `.id` is the publication row and which also lists team-library playlists, which reconcile rejects with `422 playlist_not_in_hub`). `hubs scaffold` reconciles the playlists it creates itself on both paths (MIO-4167; the client-side path only from the release after `v0.23.0`), so this is the heal path. Created lessons land unpublished unless the file AND playlist are each already published |
...
| `achievements` | `create` `list` `retrieve` `update` `archive` — team-scoped badge definitions. **SPLIT GATE**: these five need only the backend's global `ACHIEVEMENTS_ENABLED` setting; the hub-scoped verbs below additionally need `settings.achievements.enabled` on the hub (flip it with `mio hubs update <hub> --settings-json '{"achievements":{"enabled":true}}'` — an allowlisted key, read-modify-write). Any closed gate answers a **generic 404 (exit 4) by design** — it is indistinguishable from a missing resource, so do not chase it as a routing bug. `create` requires `--title`; `--appearance-json` is forwarded wholesale (backend validates strictly). `--award-mode manual` (default) is grant/revoke/restore only; `--award-mode rule` additionally needs `--rule-type` `--rule-criteria` plus either `--rule-threshold` or `--rule-content-node-ids` (mutually exclusive — the backend 422s the combination, the CLI does not block it client-side; neither `--rule-type` nor `--rule-criteria` has a documented enum, so the CLI names today's accepted values in `--help` as information only, not a client-enforced list). Both flags are `create`/`update`-shared and follow PATCH partial-update semantics on `update` — but there is **no null-clearing flag for them yet**, so flipping `--award-mode` back to `manual` does not itself clear a previously-set rule piece in the same command. Setting the pieces is necessary but **not sufficient** — see `achievements rule` below, the badge still cannot award anyone until that runs |
| `achievements offerings` | `list` `attach` `detach` — which badges a hub offers (`--hub`); `detach` needs `--yes` in non-interactive shells |
| `achievements rule` | `set` `delete` — compile/persist or unbind a **hub-scoped** award rule (`PUT`/`DELETE .../hubs/{hub}/achievements/{id}/rule`; there is no team-scoped variant, it 404s). A badge does not award anyone until this has been run — creating the definition and `offerings attach` are both necessary but not sufficient. `set` **defaults to a preview**: without `--confirm` nothing is persisted and the CLI prints the qualifying-member count in plain words (hedged as "at least N" when the backend's `preview_count_is_lower_bound` is true); pass `--confirm` to actually persist the rule and its segment. `--notify-on-backfill` opts the pre-existing qualifying ("backfill") cohort into award notifications on persist (default: silent). `delete` needs `--yes` in non-interactive shells and stops the badge awarding anyone new; existing earns are unaffected |
| `achievements grant/revoke/restore` | manual earns, hub-scoped; `--contact-id` is the **GLOBAL** contact id — capture it with `mio contacts retrieve <team-contact-id> -o plain --jq .contact_id` (the CLI flattens attributes, so the field is top-level `.contact_id`; the row's `.id` is the team-contact id and 404s). `grant` is idempotent (re-grant returns the same earn); granting over a REVOKED earn is a 409 — use `restore`. `revoke` needs `--yes`; its `--reason` travels as the `?reason=` query param, and its 204 is **idempotent — it does not confirm an earn existed** (a nonexistent earn or wrong contact id answers the same 204; the CLI's message says so). **A wrong contact id never 404s on these verbs**: grant answers 422 (not an active member), revoke 204, restore 409 (no earn exists) — the CLI appends the namespace hint on exactly those answers, while a 404 means gates/achievement/hub. `grant --reason` is accepted by the API schema but the Phase 1 backend records `award_reason="manual"` regardless (revoke/restore reasons ARE persisted). `grant` is **manual-mode only** (`422 achievement_award_mode_invalid` on a `rule` badge) — for a `rule` badge use `achievements override` instead |
| `achievements override` | audited override-grant for a **`rule`-mode** badge — the mirror image of `grant`: requires `award_mode="rule"` (422s the same code as `grant` does, in the opposite direction) **and** a rule already bound via `achievements rule set --confirm` (404 `achievement_rule_not_found` otherwise — reads like a missing badge, isn't one). It's the deterministic way to prove a rule badge actually awards, since the automatic engine (SegmentJoined fast path / backfill / sweeper) is background-only with no HTTP route to trigger. `--contact-id` (GLOBAL, same capture as above) and `--reason` are both **required**; unlike grant/revoke/restore, `--reason` is mandatory and the CLI rejects an empty or whitespace-only value client-side before any request (the backend's `AuditReason` validator would 422 it anyway — this is the one place client-side validation earns its keep, because the server's rule is unambiguous). A previously-revoked earn is a 409 (`achievement_already_revoked`) — use `restore`, not `override`, to reactivate one |
...
- **`products prices` takes the product id as a positional argument**, not `--product`:
  ...
  `--amount`, `--currency` and `--type` (`one_time`|`recurring`) are all required; `--interval` **and** `--interval-count` are required when `--type=recurring`. The optional label flag is `--name` (there is no `--nickname`), alongside `--description`, `--is-active` and `--trial-period-days` (free-trial days, 0–2147483647, 0 = no trial; checkout applies it to a `recurring` price only — the API also stores it on a `one_time` price, where it does nothing). Keep a trial at **730 days or less**: Stripe documents a 730-day maximum for a Checkout free trial, the API does not check it, and checkout hands the stored value to Stripe unchanged. Amount, currency, type, interval, interval count and trial length are **fixed after create** (`prices update` takes only `--name`/`--description`/`--is-active`): to change one, create a new price and deactivate the old one (MIO-4159).
