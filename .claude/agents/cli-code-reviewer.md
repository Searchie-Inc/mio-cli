...
- On a branch cut before MIO-2995, a bare `go test ./...` fails two tests from credential leakage, not from the code, and `XDG_CONFIG_HOME=$(mktemp -d)` makes them green. On a tree whose `cmd/main_test.go` has a `TestMain`, the tests isolate themselves, so the same two failures are a real regression of that isolation (`cli-prime` §3.1). Determine which tree you are looking at before reporting a failure.
