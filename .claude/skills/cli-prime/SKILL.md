...
- **On a tree that has that `TestMain`, a bare `go test ./...` is green, and these two failing is a REGRESSION**: the isolation broke. Read the `TestMain:` lines on stderr, which name the part that broke. Do not wave it off as environmental.
