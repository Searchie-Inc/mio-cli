...
- discussions: `list` GET `/discussions` (admin view: includes drafts + soft-deleted; `filter[space_id]`, `filter[is_broadcast]`; cursor pagination in a BARE `meta.next_cursor`/`meta.has_more`, NOT the `meta.page.*` envelope the other lists use)
