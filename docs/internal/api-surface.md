...
- CORRECTION (MIO-2567): an admin policies READ **does** exist — `GET /api/v1/teams/{team_id}/hubs/{identifier}/policies` (`admin_get_hub_policies`, MIO-2394), owner-only and tenant-scoped, reporting BOTH documents with the ACTUAL stored gate state (unlike the member-portal GET, which forces `enabled=true` for public display). This file previously said there was none, and that the only `/policies` GET was the member-portal route. It takes the SAME owner credentials as `policies gate` (identical `get_current_user` + `require_team_owner`). The CLI still ships no `hubs policies get|list` verb — filed as **MIO-2815** — so reading the stored content/version/gate needs a raw call for now, which is how an operator checks whether a scaffold resume reverted their hand-edited ToS
