...
- `list`     GET `/api/teams/{team_id}/hubs`
  - output includes the SAME derived convenience booleans as `retrieve`, injected per item (MIO-2991). They were missing here until then, so a caller enumerating hubs read `null` — and since `registration_enabled` is fail-closed, `null` is indistinguishable from a genuine `false`. `--raw` bypasses them, as on every path.
...
- tree: `get` GET `…/pages/{id}/tree?audience=author&resolve=true` (draft_version = OCC token);
  ...
  - CLIENT-SIDE PRE-FLIGHT (`cmd/pages_tree_validate.go`, MIO-2537): a pre-order walk rejects the three render-fatal shapes the API accepts with a 200 — a non-numeric `settings.weight`, a blank/non-string `template`, and a content `value` placed under `settings` on one of the seven kinds that read the TOP-LEVEL `node.value` (MIO-2575; `progress-ring` is exempt, being the sole reader of `settings.value`) — with ExitUsage and no HTTP. It is deliberately conservative; the rest of the render contract (the `surface.background` enum, the value-bearing kind vocabulary, `surface.gradient` as a SIBLING of `background`) is documented rather than validated, because the frontend is the authoritative reader and a guessed shape would reject valid trees. See `cmd/skills/content/mio-skill.md` § "The page-tree render contract" and AGENTS.md § "Page-Tree Render Contract"; the catalog-derived vocabulary lists in the skill are pinned against the embedded catalog by `TestSkillDocIsGeneratedFromCatalog` (MIO-2539/2663/2664).
...
- upload   POST `…/files` → presigned PUT (meta `upload_url`) → POST `…/files/{id}/finalize`; auto-multipart (init `…/files/multipart`, part `…/files/{id}/multipart/{upload_id}/parts/{n}`, complete `…/complete`) when large or `--multipart`; `--title` `--mime-type` `--folder-id` `--wait` `--timeout` `--part-size-mb`. **`--wait` and video transcoding (MIO-3001):** `--wait` blocks until upload AND transcode are done, but a null `status_transcode` means two different things and only one of them is "nothing to wait for". A freshly finalized VIDEO reads null for a window — the backend's finalize sets `status_upload: READY` and EMITS `MediaUploaded`; a best-effort handler enqueues the transcode job, and only that job sets `status_transcode` — so returning on the first null poll hands back an untranscoded file. `--wait` therefore keeps polling while `mime_type` is `video/*` (an exact proxy for the backend's own `asset_kind`, which is not serialized). It does NOT wait indefinitely: the transcode dispatch is gated on `FEATURE_MEDIA_VIDEO_ENABLED`, which defaults False and is false in PRODUCTION until P10, so on those backends the status stays null FOREVER and waiting for it would burn the whole `--timeout` and then FAIL. **The exact contract, which is a bound and NOT a guarantee:** the wait for a transcode to *appear* is capped at 30s (measured from `status_upload` reaching READY, and further capped by `--timeout`, whichever is smaller); once a transcode does appear, `--wait` follows it to READY under `--timeout`. Both caps are checked at POLL BOUNDARIES, so the real wall-clock bound is the cap plus up to one 2s poll interval — budget `--timeout 1s` and expect ~2s, not 1s. If nothing appears within that window the command returns **exit 0** — the upload genuinely succeeded — and warns on stderr, naming the window it actually waited. Do NOT read that warning as proof video is disabled: a backed-up transcode queue produces it too (`MEDIA_WORKER_MAX_JOBS` is 10), and the file may transcode moments later. Re-check with `mio media files retrieve <id>` rather than treating exit 0 as "transcoded". Non-video is unaffected — image/pdf/**audio** all have a permanently-null transcode status and return as soon as the upload is READY. A file reporting NO `mime_type` cannot be classified: it returns without waiting and says so. A FAILED transcode is still an error. `--wait` also blocks on `status_transcribe`, and that field has TERMINAL values that are neither null nor READY: `NOT_APPLICABLE` (literally `"READY" if words else "NOT_APPLICABLE"`, in both the auto path `app/media/transcription_service.py` and the admin re-transcribe path — the NORMAL outcome for a video with no speech) and `REJECTED`. Those are treated as settled; waiting for them to become READY would burn the whole `--timeout` and exit 1 on an upload that fully succeeded. `PENDING` — the only in-flight value this field actually takes — is still waited on, and `FAILED` is still an error. (MIO-2571's "null means not applicable" reading is unchanged — this is about non-null terminal values.)
