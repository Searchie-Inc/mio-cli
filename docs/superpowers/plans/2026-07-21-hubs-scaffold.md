...
- [ ] **Task 17 — Step 6 Playlists+items:** per O1 decision (Task 0a): match-or-create playlist; `items add` per file_id with item-level skip (option a/b) or skip-whole-step (option c). Test: reflects the O1 decision; `published_at` set unconditionally on publish; `visibility:public`.
