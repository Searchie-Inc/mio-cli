...
7. **Homepage** — `pages create` (`buildPageAttrs`, capture `page_id`) → `catalog.InstantiateTemplate(tmpl.homepage)` (the existing pages-catalog applier, callable as a library) → `client.Update`/tree-set at `pagesTreePath(page_id)` with `--if-match 0` on first set (or the captured `draft_version` on resume). Node settings run through the MIO-2537 pre-flight validation once it lands.
...
- **Extracted builders** — the real work of I1. Today the write logic is welded into cobra `RunE`s and shaped `(cmd, attrs)` reading flags; it is **not** callable from scaffold. The plan extracts pure, data-driven functions and has both the original command and scaffold call them:
  - `applyHubBlobs(ctx, client, hubID, patches) error` — extract the ~90-line RMW block from `hubsUpdateCmd.RunE` (`cmd/hubs.go`).
  - `buildSpaceAttrs`, `buildHubConfigAttrs`, `buildHubMediaPublishAttrs`, `buildPageAttrs`, `buildHubCreateAttrs` — extracted from their `set*Attrs`/`apply*Options` counterparts, taking template data instead of a `*cobra.Command`.
  - Reuse as-is (already pure/callable): path builders (`hubsPath`, `spacesPath`, `hubPlaylistsPath`, `playlistItemsPath`, `pagesPath`, `pagesTreePath`), `deepMergeMap`/`attrMap`/`deleteAtPath`, `validateNavigationBlob`/`validateNavigationHrefs`, `validateBlobKeys` (in strict mode), `catalog.InstantiateTemplate`, and `client.Create/Update/Retrieve/List`.
...
- **O1 — playlist idempotency key.** Playlists have no server slug. Options: (a) match by exact title within the hub (title collisions possible), (b) write a scaffold marker into the playlist `meta`/description and match on it, (c) accept that playlists are create-only and resume mode skips the playlist step if any playlist already exists on the hub. Leaning (b) if `meta` is writable on playlist create, else (c). Resolve during planning.
