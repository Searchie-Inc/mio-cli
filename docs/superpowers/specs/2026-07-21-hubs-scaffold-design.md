...
Almost every step consumes a server-assigned id minted by an earlier step (hub id → all; contact-attr definition id → `hub-config create` body; playlist id → `items add` + `hub-playlists publish`; page id → `tree set`). The orchestrator threads a mutable **`scaffoldContext`**: `{client, teamID, hubID, hubSlug, isPrivate, spaceIDsBySlug, defIDsBySlug, playlistIDsByKey, homePageID, homeDraftVersion}`. It resolves auth/team **once** (not per step) and fills the hub id at step 1; each later step reads what it needs and writes back the ids it mints. **Resume mode caveat:** `--hub <id>` short-circuits `ResolveHub` (an id-shaped value returns unchanged, no lookup), so scaffold **must GET the hub** after resolving `--hub` to populate `hubSlug` (required by the navigation href validators in step 2) and `isPrivate` — a bare id alone leaves `hubSlug` empty and nav validation would fail. This context, not "each step in isolation," is what makes the pipeline runnable — steps 2-8 cannot form their request without it.
...
**Lookup must be exhaustive, not first-page.** `client.List` returns a single cursor-paginated page — a naive one-page "list and skip-if-exists" can miss an existing space/def/playlist beyond page 1 and then create a duplicate (or 409). So each pre-check either (a) uses a **server-side filter** where one exists (e.g. page by `filter[slug]` if supported), or (b) **follows cursors to exhaustion** before deciding create-vs-skip. The plan must pick per resource; a first-page-only match is a bug.

**Recovery contract:** scaffold does **not** roll back. On any step failure it prints (a) the hub id, (b) `mio hubs scaffold --hub <id> --template <t>` to resume. Resume **converges for the steps with a real lookup key** (blobs — whole-hub PATCH; spaces by slug; defs by slug; homepage by page slug + `draft_version`), given exhaustive matching above. **Playlists/items are gated on O1** — until that key is pinned, playlist convergence uses the guaranteed-safe fallback (O1 option c: skip the playlist step if any playlist already exists on the hub). This replaces the earlier (incorrect) blanket "re-run just converges" claim, which failed at step 1's non-idempotent create.
...
8. **Publish** *(only with `--publish`)* — PATCH the hub with **`is_private: false`** (the CLI's `--published` flag maps to backend `is_private=false`; `published` is a *derived output* field, not a writable attribute — do NOT send `published:true`). Use `buildHubUpdateAttrs`'s published→is_private inversion. Echo URL + state.
...
- **Extracted builders** — the real work of I1. Today the write logic is welded into cobra `RunE`s and shaped `(cmd, attrs)` reading flags; it is **not** callable from scaffold. The plan extracts pure, data-driven functions and has both the original command and scaffold call them:
  ...
  - **RunE-welded extractions** (no existing helper — logic is inline in a `RunE`, so these are true extractions, not renames; all feed O2's blast-radius count): `buildHubCreateAttrs` + `buildHubUpdateAttrs` (incl. the `--published`→`is_private=false` inversion) from `hubsCreateCmd`/`hubsUpdateCmd.RunE`, `applyHubPolicies` from `hubsPoliciesUpdateCmd.RunE`, `buildAttrDefCreateAttrs` from the contact-attributes def create (`cmd/contactattributes.go`), and `buildPlaylistCreateAttrs` from the playlist create (`cmd/media.go`).
