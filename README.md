...
| `teams` | `create` (`--name` and `--slug`, both required; needs a user access token, since the API refuses an API key here with 403), `list`, `retrieve`, `update` (`--name` only; the slug is fixed at creation), `delete`, `switch` (server-side switch + updates local context); `members list/add/remove` |
